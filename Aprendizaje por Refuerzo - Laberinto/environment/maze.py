...
class Maze:
    """ Clase que representa el laberinto. Aquí está el entorno en el que el agente aprende y juega. """

    actions = [Action.MOVE_LEFT, Action.MOVE_RIGHT, Action.MOVE_UP, Action.MOVE_DOWN]  
    ...
    def __init__(self, maze, start_cell=(0, 0), exit_cell=None):
        """ Constructor: crea un nuevo laberinto """
        self.maze = maze  
        ...
        if self.__exit_cell in self.empty:
            self.empty.remove(self.__exit_cell)


        if self.__exit_cell not in self.cells:
            raise Exception("Error: exit cell fuera del laberinto")
        if self.maze[self.__exit_cell[::-1]] == Cell.OCCUPIED:
            raise Exception("Error: exit cell está bloqueada (pared)")
